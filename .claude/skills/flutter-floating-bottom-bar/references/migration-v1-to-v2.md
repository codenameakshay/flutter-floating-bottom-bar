...
- **`barColor: Colors.white` becomes `BottomBarThemeData.barDecoration.color`.** Don't drop `borderRadius` from the decoration — keep both `layout.borderRadius` and `theme.barDecoration.borderRadius` matching. Precedence: if an explicit `layout` is passed to `BottomBar` **or** the resolved theme has a non-null `layout`, `layout.borderRadius` wins outright — even `BorderRadius.zero` overrides the decoration. Otherwise the theme's `barDecoration.borderRadius` is used. Keeping both in sync avoids a zero radius silently clobbering a rounded decoration.
