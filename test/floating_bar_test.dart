...
void main() {
  ...
  testWidgets('theme-provided Motor motion updates the controller', (
    ...
    Widget appWithMotion(BottomBarMotion motion) {
      return MaterialApp(
        theme: ThemeData(extensions: [BottomBarThemeData(motion: motion)]),
        home: Scaffold(
          body: BottomBar(
            controller: controller,
    ...
    await tester.pumpWidget(
      appWithMotion(
        const BottomBarMotion.motor(motor.Motion.linear(Duration(seconds: 1))),
      ),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets(
    'back action keeps a fixed minimum 48 pixel target while hidden',
    (tester) async {
      final controller = BottomBarController();
  ...
  testWidgets(
    'back action accepts taps in the corners of its 48 pixel target',
    (tester) async {
  ...
  testWidgets(
    'user tooltip and semantics labels override directional defaults',
    (tester) async {
      final controller = BottomBarController();
      final semantics = tester.ensureSemantics();
      try {
  ...
  testWidgets(
    'visibility callbacks fire exactly once per target state change',
    (tester) async {
      final controller = BottomBarController();
      final visibilityEvents = <bool>[];
      var shownCount = 0;
  ...
  testWidgets('scrollables inside the floating child do not drive visibility '
    ...
    await tester.drag(
      find.byKey(const Key('body-list')),
      const Offset(0, -400),
    );
    await tester.pumpAndSettle();
    ...
    await tester.drag(
      find.byKey(const Key('child-list')),
      const Offset(0, -80),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets(
    'scrollToEnd collapses the header and scrolls the body to bottom',
    (tester) async {
      final controller = BottomBarController();
      final nestedKey = GlobalKey<NestedScrollViewState>();
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.drag(
        find.byKey(const Key('descendant-list')),
        const Offset(0, -180),
      );
      await tester.pumpAndSettle();
